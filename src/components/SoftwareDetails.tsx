...
export default function SoftwareDetails({ 
  ...
  const matchedLicense = userLicenses && userLicenses.find((lic: any) => 
    lic.productId === product.id || 
    lic.productId === (product as any).mappedPlanId || 
    (product.name && lic.productName && lic.productName.toLowerCase().includes(product.name.toLowerCase()))
  );
                ...
                {isOwned ? (
                  <div className="space-y-3">
                    <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 text-center">
                      <span className="text-xs font-bold text-emerald-800 font-mono tracking-wider uppercase flex items-center justify-center gap-1.5">
                        <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
                        ✓ LIFETIME LICENSE ACTIVE
                      </span>
                      {matchedLicense && (
                        <div className="mt-2 text-center">
                          <span className="text-[10px] text-slate-500 block uppercase font-mono tracking-tight">Active License Key:</span>
                          <span className="text-[11px] text-slate-700 bg-white border border-slate-150 py-1.5 px-3 rounded-xl inline-block mt-1 font-mono font-bold break-all select-all">
                    ...
                    <button 
                      ...
                      className="w-full py-4.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-2xl text-xs uppercase shadow-md hover:shadow-emerald-500/10 transition-all flex items-center justify-center gap-2 cursor-pointer border-0"
                    >
                      <Download className="w-4 h-4" />
                      <span>DOWNLOAD FULL BUILD SETUP (.EXE)</span>
                    </button>

                    <button 
                      onClick={() => onPageChange('portal')}
                      className="w-full py-3 bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-700 font-extrabold rounded-xl text-xs uppercase transition flex items-center justify-center gap-1.5 cursor-pointer"
                    >
