...
export default function Pricing({ 
  ...
  // Find matching product from products array or fall back to default properties
  const matchedProduct = products?.find(p => p.id === cartItem?.id || p.id === cartItem?.selectedPlanId);

  const rawPrice = cartItem?.price !== undefined 
    ? cartItem.price 
    : (matchedProduct?.price !== undefined ? matchedProduct.price : 999);

  const rawOriginalPrice = cartItem?.originalPrice !== undefined
    ? cartItem.originalPrice
    : (matchedProduct?.originalPrice !== undefined ? matchedProduct.originalPrice : 2499);

  const resolvedFeatures = cartItem?.features || matchedProduct?.features || [];
  ...
  const handleAddToCart = (planId: string) => {
    ...
    setCartItem({
      ...
      price: foundProduct?.price || (planId === 'prod-billing-enterprise' ? 2999 : 999),
      originalPrice: foundProduct?.originalPrice || (planId === 'prod-billing-enterprise' ? 4999 : 2499),
              ...
              <div className="bg-slate-900 border-2 border-blue-500 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl relative overflow-hidden" id="custom-selected-solution-card">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/85 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="px-2 py-0.5 bg-blue-500/15 border border-blue-500/35 text-blue-400 text-[10px] rounded font-mono font-bold uppercase tracking-wider">
                        {cartItem?.isSolution ? 'Custom Solution' : 'Lifetime Software License'}
                      </span>
                  ...
                  <div className="flex items-baseline gap-1.5 bg-slate-950 px-4 py-2 rounded-xl border border-slate-800 self-start sm:self-center">
                    <span className="text-[11px] font-mono text-slate-400 font-bold uppercase tracking-wider">Price:</span>
                ...
                <div className="pt-4 border-t border-slate-800/60">
                  <p className="text-[11px] text-slate-400 leading-normal max-w-lg">
                    This selection covers a lifetime license key with automatic system updates. Registered database schemas and design tools are fully enabled for offline POS lanes execution.
                  </p>
                </div>
              </div>
            </div>
