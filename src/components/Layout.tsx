...
export default function Layout({ 
  ...
  const [announcementMsg, setAnnouncementMsg] = useState('🇮🇳 Special Offer: Desktop GST Billing Lifetime License now only ₹999 / Free Trial Enabled');
          ...
          <div className="flex items-center justify-between h-20">
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-mono tracking-normal text-slate-500 uppercase -mt-1 font-bold">
            ...
            <nav className="hidden lg:flex items-center gap-2">
              {navItems.map((item) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-sm text-xs font-black uppercase tracking-widest transition-all duration-200 ${
                    currentPage === item.id 
            ...
            <div className="hidden lg:flex items-center gap-3">
              {user ? (
                  ...
                  >
                    ...
                    <div className="flex flex-col text-left font-sans">
                      ...
                      <span className="text-[8px] text-slate-400 font-bold leading-none mt-1 uppercase tracking-wide">
                        {user.role === 'admin' ? '🇮🇳 SYSTEM ADMIN' : 'Customer Account'}
                      </span>
                    </div>
                  </div>
                ...
                <button
                  onClick={() => handleNavClick('portal')}
                  className="px-6 py-2 bg-white text-[#0F172A] font-black text-xs uppercase tracking-tight rounded-sm hover:bg-slate-100 shadow-[4px_4px_0px_#2563EB] hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 transition-all cursor-pointer"
          ...
          <div className="lg:hidden border-t border-slate-800 bg-[#0F172A]/95 backdrop-blur-md px-4 py-4 space-y-2 transition-all">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => handleNavClick(item.id)}
                className={`w-full text-left px-4 py-3 rounded-sm text-xs font-black uppercase tracking-widest transition-all ${
                  currentPage === item.id 
